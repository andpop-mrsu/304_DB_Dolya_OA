...
CREATE TABLE Groups (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    program_id INTEGER NOT NULL,
    name TEXT NOT NULL,                -- номер группы, например '303'
    graduation_year INTEGER NOT NULL CHECK (graduation_year >= 1900),
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%d', 'now')),
    UNIQUE (name),                     -- номер группы уникален
    FOREIGN KEY (program_id) REFERENCES Programs(id) ON DELETE CASCADE
);
...
CREATE TABLE Students (
    ...
    student_card_number TEXT NOT NULL UNIQUE,  -- явный номер студенческого
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%d', 'now')),
...
INSERT INTO Students (group_id, study_plan_id, full_name, birth_date, gender, student_card_number) VALUES
    (1, 1, 'Иванов Иван Иванович', '2002-05-15', 'M', '303-ИИИ-001'),
    (1, 1, 'Петрова Анна Сергеевна', '2001-11-23', 'F', '303-ПАС-002'),
    (3, 3, 'Сидоров Алексей Владимирович', '2001-08-30', 'M', '304-САВ-001');
